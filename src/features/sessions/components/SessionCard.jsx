export default function SessionCard({ session, deleteSession, onEdit }) {
        ...
        <div className="flex flex-col bg-[#111114] rounded-2xl overflow-hidden border border-[#2a2a2e] hover:border-[#7bf4e1]/30 transition-all">
            <div className="p-3.5 md:p-5 flex flex-col gap-3 flex-1">
                ...
                <div className="flex justify-between items-center">
                    <h3 className="text-sm md:text-base font-bold text-[#f9f9f9] line-clamp-1 flex-1">{session.name}</h3>
                ...
                <div className="flex-1 flex flex-col gap-2">
                    {session.parameters.length > 0 ? (
                        session.parameters.map((param) => (
                            <div key={param.id} className="flex items-baseline justify-between gap-2">
                                <span className="text-[13px] md:text-sm text-[#f9f9f9]/80 font-medium">{param.name}</span>
                                <span className="text-[13px] md:text-sm text-[#7bf4e1] font-semibold tabular-nums">
                                    {param.value || "—"}
                                    {param.units && <span className="text-[10px] text-[#535458] ml-0.5 font-normal">{param.units}</span>}
                                </span>
                            </div>
                        ))
                    ) : (
                        <p className="text-[#535458] text-xs italic">No data</p>
                    )}
                </div>

                {/* Date */}
                <div className="pt-2 border-t border-[#2a2a2e] mt-auto flex items-center gap-1.5 text-[10px] md:text-xs text-[#535458]">
